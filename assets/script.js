...
var displayLocation = function () {
  ...
  var coordinates = data.events.filter(
    (list) =>
      list.categories[0].title === category && list.geometries[0].coordinates
  );
  ...
  if (category === "Sea and Lake Ice") {
    countEl.textContent =
      "There are " + eventCount + " " + category + " " + "events reported!";
  } else {
    countEl.textContent =
...
function init() {
  ...
  var currentSelectedCategory = JSON.parse(
    localStorage.getItem("currentSelectedCategory")
  );
